...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.product-image').forEach((img) => {
        img.addEventListener('click', () => {
            ...
            localStorage.setItem('productCategory', 'womenProducts');
   ...
   document.querySelectorAll('.add-to-cart-text').forEach((text) => {
    text.addEventListener('click', () => {
        const womenproductId = parseInt(text.dataset.productId);
        console.log("Adding to cart, Product ID:", womenproductId);
        addtoCart(womenproductId, 'womenProducts');
        let adddedNumber = 1;
